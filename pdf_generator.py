...
class PDFGenerator:
    ...
    def generate_invoice(self, invoice, customer):
        ...
        elements.append(Paragraph(f"Invoice #{invoice.invoice_number}", self.styles["Heading2"]))
        elements.append(Paragraph(f"Date: {invoice.date}", self.styles["Normal"]))
        elements.append(Paragraph(f"Due Date: {invoice.due_date}", self.styles["Normal"]))
        elements.append(Spacer(1, 20))
